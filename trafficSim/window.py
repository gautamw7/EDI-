...
class Window:
    ...
    def draw(self):
        ...
        for road in self.sim.roads:
            # Draw road segment
            self.line(road.start, road.end, color=(0, 0, 0))            # Draw intersection (if applicable)
            if isinstance(road, Interconnect):
                self.circle(road.end, radius=5, color=(255, 0, 0), filled=True)  # Example: Red circle for intersection
